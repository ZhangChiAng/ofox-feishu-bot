...
关注列表通过“关注管理”维护，并由所有有权使用机器人的用户全局共享。卡片支持逐项关注/取消、提供商与关键词组合筛选、分页和二次确认后清空；关键词忽略大小写并匹配模型名称、ID 与提供商。若模型目录暂不可用，已有关注仍可查看、取消或清空，添加功能会暂时禁用。每日发现新增模型时，图片报告后会附带分页快捷卡片，只提供逐项操作。
